...
class PostTypeGenerator
{
    ...
    function register_post_type()
    {
        ...
        $args = array(
            "label" => $this->pluralize($n),
            'singular_name' => $n,
            'labels' => array(
                'add_new_item' => _x('Add New ' . $n, strtolower($n))
            ),
            "public" => true,
            "publicly_queryable" => true,
            "query_var" => true,
            #"menu_icon" => get_stylesheet_directory_uri() . "/article16.png",
            "rewrite" => array('slug' => strtolower($this->pluralize($n))),
    ...
    function pluralize($name) {
        ...
        if ($lastChar == 'y') {
            return ucwords(substr($name, 0, strlen($name)-1)) . 'ies';
        } else if ($lastChar != 's'){
                    ...
                    { // function that displays the form fields
                        ...
                        foreach ($inputs as $name => $type) {
                            ...
                            $lookup = array(
                                ...
                                "date" => "<input type='date' name='$id_name' value='$value' class='widefat' />",
                                "time" => "<input type='time' name='$id_name' value='$value' class='widefat' />",
...
// $product = new PostTypeGenerator("movie");
